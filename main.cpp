...
// const char* versionPattern = "7.40";
// std::ifstream executable("Fortnite.exe IK ITS NOT YET BUT WORK 7.4 RN", std::ios::binary);
// std::string fileContents((std::istreambuf_iterator<char>(executable)),
	//	std::istreambuf_iterator<char>());

// if (fileContents.find(versionPattern) != std::string::npos) {
	// Version matches
// }
// else {
...
//}
//
// const char* versionPattern = "8.51";
// std::ifstream executable("Fortnite.exe IK ITS NOT YET BUT WORK 7.4 RN", std::ios::binary);
// std::string fileContents((std::istreambuf_iterator<char>(executable)),
	//	std::istreambuf_iterator<char>());
...
//}
//
// const char* versionPattern = "10.40";
// std::ifstream executable("Fortnite.exe IK ITS NOT YET BUT WORK 7.4 RN", std::ios::binary);
// std::string fileContents((std::istreambuf_iterator<char>(executable)),
	//	std::istreambuf_iterator<char>());
...
//}
//
// broken rn but work 7.40 manually change
// ugly kysS
void SetupWindow()
{
	GameWnd = FindWindowW(NULL, TEXT("Fortnite (protected by FGuard)")); 
...
void actorLoop() {
	...
	uintptr_t uWorld = Driver::read<uintptr_t>(pid, BaseAddr + 0x5C80950); //update
	if (!uWorld) {
		return;
	}
	...
	uintptr_t OwningGameInstance = Driver::read<uintptr_t>(pid, uWorld + 0x160); //updated
...
void renderLoopCall() {
	...
	{
		if (ImGui::Begin(z(" Novahaxx Private - ud man build "), 0, ImGuiWindowFlags_::ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_::ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoSavedSettings))
			ImGui::SetNextWindowSize(ImVec2({ 700, 650 }), ImGuiSetCond_FirstUseEver); {
				ImGui::SetWindowSize(ImVec2(450, 790));
				ImGui::Text(z("Visuals"));
				ImGui::Checkbox(z("Box ESP"), &Settings::Box);
			
				ImGui::Text(z("Aimbot"));
				ImGui::Checkbox(z("Visible Check"), &Settings::VisibleCheck);
				ImGui::Text(z("Aimbot Key: "));
				ImGui::SameLine(90.f);
				...
				ImGui::Checkbox(z("Mouse Aimbot"), &Settings::Aimbot);
				ImGui::Checkbox(z("Silent Aimbot"), &Settings::SilentAim);
